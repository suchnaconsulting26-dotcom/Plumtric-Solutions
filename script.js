...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const animatedElements = document.querySelectorAll(
    ".service-card, .why-card, .feature, .team-card",
  );
  ...
  if (contactForm) {
    contactForm.addEventListener("submit", function (e) {
      ...
      window.open(
        `https://wa.me/919316808696?text=${whatsappMessage}`,
        "_blank",
      );
  ...
  const handleLoader = () => {
    ...
    if (loader && !loader.classList.contains("fade-out")) {
      ...
      }, 400); // Reduced from 600ms to 400ms for faster response
    }
  };

  // Improved loader handling for all page states
  if (document.readyState === "complete" || document.readyState === "interactive") {
    // Page is already loaded or interactive, remove loader immediately
    handleLoader();
  } else {
  ...
  // Fallback: Force remove loader after 2 seconds if load event gets stuck
  setTimeout(handleLoader, 2000);

  // ================================================
  // MODAL FUNCTIONALITY FOR PROFILES PAGE
  // ================================================
  const idCards = document.querySelectorAll('.id-card[data-modal]');
  const modals = document.querySelectorAll('.modal');
  
  // Open modal when clicking on ID card
